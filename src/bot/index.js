...
import { startScanner, stopScanner, getWatchlistSize } from '../scanner/index.js';
...
import { getClient, getWalletClient } from '../core/chain.js';
import {
    ...
    res.on('end', () => {
      try {
        ...
        if (!parsed.ok && fallbackMarkdown) {
          // Rich Message API not available, fallback to Markdown
...
export async function setupBot() {
  ...
  bot.command('start', (ctx) => {
    const html =
      `<h3>🤖 Alpha Hunter - Pons Sniper (V2)</h3>` +
      ...
      `<tr><td><code>/summary</code></td><td>Trading summary</td></tr>` +
      `<tr><td><code>/clear_logs</code></td><td>Clear old logs</td></tr>` +
      `</table>` +
      `<p>${env.DRY_RUN ? '🧪 <i>DRY RUN MODE</i>' : '💰 <b>LIVE MODE</b>'}</p>`;

    const fallback =
      `🤖 *Alpha Hunter - Pons Sniper (V2)*\n\n` +
  ...
  bot.command('help', (ctx) => {
    const html =
      ...
      `<tr><td><b>bonding_curve_pct</b></td><td align="right">80</td><td><code>/set bonding_curve_pct</code></td></tr>` +
      `<tr><td><b>min_holders</b></td><td align="right">10</td><td><code>/set min_holders</code></td></tr>` +
  ...
  bot.command('set', async (ctx) => {
    ...
    const validKeys = [
      ...
      'scanner_poll_ms', 'monitor_poll_ms', 'max_open_positions', 'max_position_minutes', 'only_eth_paired',
    ];
  ...
  bot.command('summary', async (ctx) => {
    ...
    // Total PnL in ETH
    const totalPnlEth = closed.reduce((sum, p) => {
      const buy = p.buyAmountEth || 0;
      const sell = p.sellAmountEth || 0;
      return sum + (sell - buy);
    }, 0);
    ...
    const todayPnlEth = todayClosed.reduce((sum, p) => {
      const buy = p.buyAmountEth || 0;
      const sell = p.sellAmountEth || 0;
      return sum + (sell - buy);
    }, 0);
